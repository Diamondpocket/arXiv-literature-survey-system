...
def run_analysis(
    cards_path: Path | str = Path("data/paper_cards.jsonl"),
    outputs_dir: Path | str = Path("outputs"),
    taxonomy_prompt_path: Path | str | None = Path("prompts/taxonomy_generation.txt"),
    ...
    (outputs_dir / "taxonomy.md").write_text(taxonomy, encoding="utf-8")
    pd.DataFrame(comparison_rows, columns=COMPARISON_COLUMNS).to_csv(
        outputs_dir / "comparison_table.csv",
        index=False,
        encoding="utf-8",
    )
    (outputs_dir / "trend_analysis.md").write_text(trend_analysis, encoding="utf-8")

    print(f"[cluster_analysis] Wrote {outputs_dir / 'taxonomy.md'}")
    print(f"[cluster_analysis] Wrote {outputs_dir / 'comparison_table.csv'}")
    print(f"[cluster_analysis] Wrote {outputs_dir / 'trend_analysis.md'}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate taxonomy, comparison table, and trend analysis.")
    parser.add_argument("--cards_path", default="data/paper_cards.jsonl")
    parser.add_argument("--outputs_dir", default="outputs")
    parser.add_argument("--taxonomy_prompt_path", default="prompts/taxonomy_generation.txt")
