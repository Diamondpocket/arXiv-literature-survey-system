...
from ..metas.paths import RAWS_DIR
...
def fetch_papers(
    ...
    max_results: int = 100,
    years: int = 2,
    output_path: Path | str = RAWS_DIR / "papers_raw.json",
    ...
    try:
        ...
        raise RuntimeError(
            "Package `arxiv` is not installed. Run `pip install -r cfgs/pkgs/requirements.txt` first."
        ) from exc
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default=str(RAWS_DIR / "papers_raw.json"), help="Output JSON path.")
