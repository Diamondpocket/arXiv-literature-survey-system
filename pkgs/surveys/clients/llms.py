...
if load_dotenv:
    candidate_paths = [
        ...
        Path(__file__).resolve().parents[4] / "cfgs" / "envs" / ".env",
    ]
...
class LLMClient:
    ...
    def __post_init__(self) -> None:
        ...
        if env_temperature is not None:
            ...
            self.temperature = _default_temperature_for_provider(requested_model, self.base_url)
    ...
    def _create_completion(self, messages: list[Dict[str, str]], json_mode: bool) -> Any:
        ...
        while True:
            try:
                ...
                if fallback_temperature is not None and fallback_temperature != kwargs["temperature"]:
                    print(
                        f"[llm_client] Model rejected temperature={kwargs['temperature']}; "
