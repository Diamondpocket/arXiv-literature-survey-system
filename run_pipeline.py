...
from fetch_arxiv import DEFAULT_QUERY, fetch_papers
from generate_cards import generate_cards
from weekly_survey_generator import generate_weekly_digest
from workflow_metadata import STAGE_SEQUENCE, build_initial_stage_entries
...
def ensure_layout() -> None:
    ...
    (OUTPUTS_DIR / "weekly").mkdir(parents=True, exist_ok=True)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip_fetch",
        action="store_true",
        help="Skip arXiv fetching and reuse data/papers_raw.json. Useful for local smoke tests.",
    )
    parser.add_argument(
        "--card_limit",
        type=int,
...
def main() -> None:
    ...
    try:
        ...
        cards, cards_report = generate_cards(
            ...
            prompt_path=PROMPTS_DIR / "card_extraction.txt",
        ...
        run_analysis(
            ...
            taxonomy_prompt_path=PROMPTS_DIR / "taxonomy_generation.txt",
            use_llm_taxonomy=not args.no_llm_taxonomy,
        )
        analysis_stats = {
            "taxonomy_path": str(OUTPUTS_DIR / "taxonomy.md"),
            "comparison_path": str(OUTPUTS_DIR / "comparison_table.csv"),
            "trend_path": str(OUTPUTS_DIR / "trend_analysis.md"),
            "comparison_rows": count_csv_rows(OUTPUTS_DIR / "comparison_table.csv"),
        }
        ...
        mark_stage(
            ...
            "analysis",
            "completed",
            "Analysis artifacts refreshed in outputs/.",
            current=3,
            total=3,
        )
        ...
        digest = generate_weekly_digest(
            cards_path=cards_path,
            outputs_dir=OUTPUTS_DIR,
            prompt_path=PROMPTS_DIR / "weekly_digest.txt",
            topic=args.topic,
            force_deterministic=args.no_llm_weekly,
        )
        today = datetime.now().date().isoformat()
        weekly_stats = {
            "latest_path": str(OUTPUTS_DIR / "weekly_digest_latest.md"),
            "archive_path": str(OUTPUTS_DIR / "weekly" / f"{today}.md"),
